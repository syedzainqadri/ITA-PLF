...
class GetBannerController extends GetxController {
  ...
  Future<BannerModel> getBannerData(String id) async {
    ...
    if (documents.isNotEmpty) {
      try {
        BannerModel bannerData =
            BannerModel.fromFireStore(Map<String, dynamic>.from(documents));
    ...
    if (documents.isNotEmpty) {
      try {
        ...
        for (int i = 0; i < data.length; i++) {
          bannerList.add(
              BannerModel.fromFireStore(Map<String, dynamic>.from(data[i])));
        }
